...
//   if ( ms - frameStartAtMs >= 150/10 ) state = state_t::pendingFrame ;  // force clean up of any failed frame
...
//      //digitalWrite( 13, LOW ) ;
//     if ( isFalling && ms - frameStartAtMs >= 7/10 && ms - frameStartAtMs <= 11/10  ) { // spec = 9mS
...
//     if ( isRising && us - inHeaderSpaceAtUs >= 3500/10  && us - inHeaderSpaceAtUs <= 5500/10 ) { // spec = 4.5mS
//       state = state_t::inBitMark ;
//     }
//     else if ( isRising && us - inHeaderSpaceAtUs >= 1750/10  && us - inHeaderSpaceAtUs <= 3250/10  &&  ms - frameStartAtMs_last >= 90/10 &&  ms - frameStartAtMs_last <= 130/10 ) { // spec = 2.5mS / 110mS
...
//     if (  isRising ) {
//       state = state_t::inBitMark ;
//       if ( us - bitSpaceStartAtUs > 1400/10 &&  us - bitSpaceStartAtUs < 1900/10) {  //  0 = 562uS ; 1 = 1688 uS
//         // 1 found
//         bitSet( dataRaw, bitcount) ;
//       }
//       else if ( us - bitSpaceStartAtUs > 400/10 &&  us - bitSpaceStartAtUs < 750/10 ) {  //  0 = 562uS ; 1 = 1688 uS
//         // 0 found - do nothing
//       }
//       else state = state_t::pendingFrame ; // error so quit
